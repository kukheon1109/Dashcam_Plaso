...
from plaso.parsers.MP4 import non_iso
from plaso.parsers.MP4.core import *
from plaso.parsers.MP4.util import *

chunk_of_list = []

class chunk_list:
    def get_val(self):
        global chunk_of_list
        return chunk_of_list
...
class file(object):
    ...
    def open_file_obejct(self, object, mode='rb'):
        self.target_file = object
        print('[MP4]:open complete')
        return  ""

    def close(self):
        self.target_file.close()
        print('[MP4]:close complete')
        return ""
    ...
    def check_file_signature_file_object(self):
        ...
        if self.target_file.read(8) == b'\x00\x00\x00\x20\x66\x74\x79\x70':
            return b'MP4'
        return None

class Mp4File:

    def __init__(self, file_object, start=0, end=0):
        self.file_object = file_object
        self.type = 'file'
        self.child_boxes = []
        end_of_file = False
        file_object.seek(start,end)

        while not end_of_file:
            current_header = Header(file_object)
            chunk_of_list.append(current_header.type)
            current_box = box_factory(file_object, current_header, self)
            self.child_boxes.append(current_box)
            if current_box.size == 0:
                end_of_file = True
            size_Test = len(file_object.read(4))
            if size_Test != 4:
                end_of_file = True
            else:
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            for i in range(self.box_info['entry_count']):
                ...
                self.box_info['entry_list'].append(read_u32(fp))
        finally:
            fp.seek(self.start_of_box + self.size)
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            for i in range(self.box_info['entry_count']):
                ...
                self.box_info['entry_list'].append(read_u64(fp))
        finally:
            fp.seek(self.start_of_box + self.size)
